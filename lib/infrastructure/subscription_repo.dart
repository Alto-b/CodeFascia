...
class SubscriptionRepo {
    ...
    try {
      ...
      for (var element in datas.docs) {
        ...
        if (title.contains(lowercaseKeyword)) {
          // Check if title contains the lowercase keyword
          final specSubs = SubscriptionModel(
            subsId: data['SubsId'],
            title: data['title'],
            language: data['language'],
      ...
      debugPrint("Exception getting specific subscriptions: ${e.message}");
    }
    return specSubsList;
  }
  ...
  Future<List<BookingModel>> mySubscriptions(String uid) async {
    ...
    try {
      final subsdatas = await FirebaseFirestore.instance
          .collection("bookings")
  ...
  Future<void> updateSubsStats() async {
    ...
    for (var element in subs.docs) {
      ...
      if (currDate.isAfter(expiryDate) &&
          (currStatus == "ongoing" || currStatus == "pending")) {
