...
class _SubscriptionBookingPageState extends State<SubscriptionBookingPage> {
  ...
  void _handlePaymentSuccess(PaymentSuccessResponse response) {
    ...
    context
        ...
        .add(BookSubscriptionEvent(data: detail, bookingId: bookId!));
  ...
  void bookSubscription(SubscriptionState state, Map<String, dynamic> data,
      Map<String, dynamic> ops) async {
    try {
  ...
  Widget build(BuildContext context) {
    context
        .read<SubscriptionBloc>()
        .add(SpecificSubsLoadEvent(SubsId: widget.subId));
                                    ...
                                    child: ListTile(
                                      ...
                                      onLongPress: () {
                                        var options = {
                                          ...
                                          'amount':
                                              '${(int.parse(state.specSubsList[0].amount) * duration[index] * 100)}',
                                          'name': 'CodeFascia',
                                          'description':
                                              state.specSubsList[0].title,
                                          'prefill':
                                        ...
                                        var data = {
                                          ...
                                          "sub_id":
                                              state.specSubsList[0].subsId,
                                          "sub_title":
                                              state.specSubsList[0].title,
                                          "sub_photo":
                                              state.specSubsList[0].photo,
                                          "sub_lang":
                                              state.specSubsList[0].language,
                                          "booking_date":
                                          ...
                                          "expiry_date":
                                              DateFormat('dd-MM-yyyy').format(
                                                  DateTime.now().add(Duration(
                                                      days: duration[index]))),
                                          "booking_amount":
                                              '${(int.parse(state.specSubsList[0].amount) * duration[index])}',
                                        };
